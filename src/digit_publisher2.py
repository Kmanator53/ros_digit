...
def collect_digits(pos = None):
	...
	if pos is None:
		...
		for i,serial in enumerate(serials):
			...
			d[i].connect()
			d[i].set_intensity(brightness)
	elif len(serials)>pos:
		d = [Digit(serials[pos])]
		d[0].connect()
		d[0].set_intensity(brightness)
	return d
...
if __name__ == '__main__':
	...
	if len(sys.argv)>1:
		digits = collect_digits(int(sys.argv[1]))
	else:
	...
	if len(digits)==0:
		...
		print(f"plus the position we were looking for is {sys.argv[1]}")
		sys.exit()
